...
var
  ...
  i: Integer;
begin
  if FileExists(GetAnySource('ISD_List_Manual.txt')) then
  begin
    TxtFileToConvert := TPopupBox.Create(nil);
    TxtFileToConvert.Items.LoadFromFile(GetAnySource('ISD_List_Manual.txt'));
    ...
    if FileExists(GetAnySource('ISD_List_Manual.txt')) then
      DeleteFile(GetAnySource('ISD_List_Manual.txt'));

    ConvertToIni := TIniFile.Create(GetAnySource('ISD_List_Manual.ini'));
    i := 0;
    repeat
      i := i + 1;
