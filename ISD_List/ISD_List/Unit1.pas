...
var
  ...
  i: Integer;
begin
  if FileExists(GetAnySource('ISD_List.txt')) then
  begin
    TxtFileToConvert := TPopupBox.Create(nil);
    TxtFileToConvert.Items.LoadFromFile(GetAnySource('ISD_List.txt'));
    ...
    if FileExists(GetAnySource('ISD_List.txt')) then
      DeleteFile(GetAnySource('ISD_List.txt'));

    ConvertToIni := TIniFile.Create(GetAnySource('ISD_List.ini'));
    i := 0;
    repeat
      i := i + 1;
