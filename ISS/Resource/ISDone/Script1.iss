...
OutputBaseFilename=ISDone_List_Auto
OutputDir=.
Uninstallable=no
...
[Files]
Source: "ISDone_List_Auto.bat"; DestDir: {tmp}; Flags: dontcopy;
...
var
  ResultCode: integer;
begin
  ExtractTemporaryFile('ISDone_List_Auto.bat');
  FileCopy(ExpandConstant('{tmp}\ISDone_List_Auto.bat'),ExpandConstant('{src}\ISDone_List_Auto.bat'),false);

  AppProcessMessages;
  Sleep(150);

  Exec(ExpandConstant('{src}\ISDone_List_Auto.bat'),'','',SW_SHOWNORMAL,ewWaitUntilTerminated,ResultCode);

  AppProcessMessages;
  Sleep(150);

  DeleteFile(ExpandConstant('{src}\ISDone_List_Auto.bat'));
  Result:=False;
end;